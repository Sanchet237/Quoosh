  ...
  return (
    <QuooshLayout showLogo={true}>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {registered && (
          <div className="rounded-lg border border-green-500 bg-green-500/10 px-4 py-3 text-sm text-green-600">
            Registration successful! Please log in.
          </div>
        )}

        {error && (
          <div className="rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-center text-sm font-medium text-red-600">
            {error}
          </div>
        )}
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-xl border-2 border-gray-200 bg-gray-50 px-4 py-3 text-base text-gray-800 placeholder-gray-400 transition-all focus:border-amber-400 focus:bg-white focus:outline-none"
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full rounded-xl border-2 border-gray-200 bg-gray-50 px-4 py-3 pr-12 text-base text-gray-800 placeholder-gray-400 transition-all focus:border-amber-400 focus:bg-white focus:outline-none"
              placeholder="••••••••"
            />
            <button
              type="button"
        ...
        <button
          type="button"
          onClick={async () => {
            await signIn("google", { callbackUrl: "/auth/google-redirect" })
          }}
      ...
      <p className="mt-6 text-center text-sm text-gray-500">
        ...
        <Link href="/auth/register" className="font-bold text-amber-600 underline hover:text-amber-500">
          Register
        </Link>
      </p>
    </QuooshLayout>
